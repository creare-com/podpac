...
class TestInterpolate(object):
    ...
    class TestInterpolation(object):
        ...
        def test_dict_definition(self):
            ...
            assert interp._config[('lon',)][0] == INTERPOLATION_DEFAULT
            ...
            assert interp._config[('lon',)][0] == 'optimal'
        ...
        def test_tuple_definition(self):
            ...
            assert interp._config[('lat', 'lon')]
            assert isinstance(interp._config[('lat', 'lon')], tuple)
            assert isinstance(interp._config[('lat', 'lon')][1][1], Rasterio)
        ...
        def test_interpolator_init(self):
            ...
            interp = Interpolation('nearest', COORDINATES)
            assert interp._config[('lat', 'lon')][1][0].method == 'nearest'

        def test_init_kwargs(self):
            
            interp = Interpolation('nearest', COORDINATES, tolerance=1)
            assert interp._config[('lat', 'lon')][1][0].tolerance == 1
            ...
            with pytest.raises(AttributeError):
                assert interp._config[('lat', 'lon')][1][0].myarg == 'tol'
