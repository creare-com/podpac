...
class TestDataSourceIntegration:
    ...
    def test_array(self):
        ...
        coord = Coordinate(
            lat_lon=(lat, lon), time=(0, 10, 11), order=["lat_lon", "time"]
        )
    ...
    def test_wcs_source(self):
        ...
        coordinates = podpac.Coordinate(
            lat=(39.3, 39.0, 64),
            lon=(-77.0, -76.7, 64),
            time="2017-09-03T12:00:00",
            order=["lat", "lon", "time"],
        )
        reprojected_coordinates = (
            podpac.Coordinate(
                lat=(45, 0, 3), lon=(-70.0, -65.0, 3), order=["lat", "lon"]
            ),
        )
        ...
        reprojected = ReprojectedSource(
            source=wcs, coordinates_source=smap, interpolation="nearest"
        )
    ...
    class TestBasicInterpolation(object):
        ...
        def setup_method(self, method):
            self.coord_src = Coordinates(
                [clinspace(45, 0, 16), clinspace(-70.0, -65.0, 16), clinspace(0, 1, 2)],
                dims=["lat", "lon", "time"],
            )
        ...
        def test_raster_to_raster(self):
            coord_dst = Coordinates(
                [clinspace(5.0, 40.0, 50), clinspace(-68.0, -66.0, 100)],
                dims=["lat", "lon"],
            )
            ...
            LON, LAT = np.meshgrid(
                coord_dst["lon"].coordinates, coord_dst["lat"].coordinates
            )
