...
class TestCompositor(object):
    ...
    def test_compositor_interface_functions(self):
        node = Compositor(sources=self.sources)
        with pytest.raises(NotImplementedError):
            node._shared_coordinates_default()
        with pytest.raises(NotImplementedError):
            node.composite(outputs=None)
        assert node.get_source_coordinates() == None
        assert node._source_coordinates_default() == node.get_source_coordinates()
