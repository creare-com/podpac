...
class Coord(tl.HasTraits):
    ...
    def _coords_validate(self, proposal):
        if not isinstance(proposal['value'], [tuple, np.ndarray, xr.DataArray]):
            raise CoordinateException("Coords must be of type tuple " +\
...
class Coordinate(tl.HasTraits):
    ...
    def __init__(self, coords=None, projection="WGS84", ctype='point',
            segment_position=0.5, bounds=[], **kwargs):
        """
    ...
    # MOVE A LOT OF THIS INTO THE COORD
        ...
        # Irregular spacing independent coordinates
            ...
            raise CoordinateException("Coordinates '" + str(val) + "' for '"\
                    + dim + "' not understood")
