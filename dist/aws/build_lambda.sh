...
docker cp "${DOCKER_NAME}":/tmp/vendored/podpac_deps.zip podpac_deps.zip
...
if [ -z $TAG ]
  then
    ...
    aws s3 cp podpac_deps.zip s3://podpac-s3/podpac/podpac_deps_$TAG.zip
    aws s3 cp podpac_dist.zip s3://podpac-s3/podpac/podpac_dist_$TAG.zip
fi
aws s3 cp podpac_deps.zip s3://podpac-s3/podpac/podpac_deps_latest.zip
aws s3 cp podpac_dist.zip s3://podpac-s3/podpac/podpac_dist_latest.zip
